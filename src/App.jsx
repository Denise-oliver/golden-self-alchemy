...
const s = {
  ...
  insightBar: { background: "linear-gradient(90deg, #0d2240, #122a4a)", borderLeft: "2px solid #C5B358", padding: "10px 16px", display: "flex", gap: 10, alignItems: "flex-start", transition: "opacity 0.5s ease", position: "relative", zIndex: 1 },
  ...
  navBtn: { flex: 1, background: "none", border: "none", color: "#4a6f8a", padding: "10px 0 12px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 },
  ...
  pilarCard: { background: "#0f2744", border: "1px solid #1a3a6c", borderRadius: 12, padding: "14px 8px", textAlign: "center", cursor: "pointer" },
  pilarLabel: { color: "#8aadcf", fontSize: 11, margin: "8px 0 2px" },
...
function RingProgress({ pct, icon, size = 80 }) {
  const r = size * 0.38, circ = 2 * Math.PI * r, fill = circ * (1 - pct / 100);
  return (
    <svg width={size} height={size} style={{ display: "block", margin: "0 auto" }}>
      ...
      <text x="50%" y="50%" textAnchor="middle" dominantBaseline="central" fontSize={size*0.28} fill="#ffd700">{icon}</text>
    </svg>
  );
}
...
function TabDashboard() {
  ...
  const save = (id) => {
    ...
    setPilares(updated); DB.set("pilares", updated); setEditing(null);
  };
      ...
      {editing && (() => {
        ...
        return (
          <Modal title={`Atualizar · ${p.label}`} onClose={() => setEditing(null)}>
            ...
            <input type="range" min={0} max={100} value={tempPct} onChange={e => setTempPct(Number(e.target.value))} style={s.range} />
            <label style={s.label}>Anotação do dia</label>
            <textarea style={s.textarea} rows={3} value={tempNota} onChange={e => setTempNota(e.target.value)} placeholder="Como está essa área hoje, amiga?" />
...
function TabFinanceiro() {
  ...
  const [form, setForm] = useState({ desc: "", valor: "", tipo: "entrada", categoria: "QUITAÇÃO/INVESTIMENTO" });
  ...
  const saveForm = () => {
    ...
    if (editItem) {
      ...
      const upd = [{ id: uid(), desc: form.desc, valor: v, tipo: form.tipo, categoria: form.categoria, data: new Date().toLocaleDateString("pt-BR") }, ...lancamentos];
      setLancamentos(upd); DB.set("lancamentos", upd);
    }
    setModal(false);
  };
    ...
    <div style={s.tabWrap}>
      ...
      <div style={s.card}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {PERFIS.map(p => (
            <button key={p.id} style={{ ...s.perfilBtn, borderColor: p.cor, background: perfilAtivo === p.id ? p.cor + "33" : "transparent", color: perfilAtivo === p.id ? p.cor : "#8aadcf" }} onClick={() => setPerf(p.id)}>{p.label}</button>
          ))}
        </div>
      ...
      {lancamentos.map(l => (
        <div key={l.id} style={s.txRow}>
          <div style={{ flex: 1 }}>
            <p style={{ margin: 0, color: "#e8f0fe", fontSize: 14 }}>{l.desc}</p>
            <p style={{ margin: 0, color: "#4a6f8a", fontSize: 11 }}>{l.data} · {l.categoria}</p>
          </div>
          <p style={{ margin: "0 12px 0 0", color: l.valor > 0 ? "#C5B358" : "#E2725B", fontWeight: "bold", fontSize: 15, whiteSpace: "nowrap" }}>{l.valor > 0 ? "+" : ""}{fmt(l.valor)}</p>
      ...
      {modal && (
        <Modal title={editItem ? "Editar Lançamento" : "Novo Lançamento"} onClose={() => setModal(false)}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {["entrada", "saida"].map(t => (
              <button key={t} style={{ ...s.btnAdd, background: form.tipo === t ? "#E2725B44" : "transparent" }} onClick={() => setForm({ ...form, tipo: t })}>{t === "entrada" ? "Entrada" : "Saída"}</button>
            ))}
          </div>
...
function TabOficina() {
  ...
  const saveProj = () => {
    ...
    const upd = editProj ? projetos.map(p => p.id === editProj.id ? { ...p, ...formProj } : p) : [{ id: uid(), ...formProj }, ...projetos];
    setProjetos(upd); DB.set("projetos", upd); setModalProj(false); setEditProj(null);
  };
  const nextStatus = (id) => {
    const upd = projetos.map(p => { if (p.id !== id) return p; const idx = STATUS_FLOW.indexOf(p.status); return { ...p, status: STATUS_FLOW[Math.min(idx + 1, STATUS_FLOW.length - 1)] }; });
  ...
  const saveIns = () => {
    ...
    const upd = editIns ? insumos.map(i => i.id === editIns.id ? { ...i, ...formIns } : i) : [{ id: uid(), ...formIns }, ...insumos];
    setInsumos(upd); DB.set("insumos", upd); setModalIns(false); setEditIns(null);
  };
    ...
    <div style={s.tabWrap}>
      ...
      {projetos.map(p => (
        <div key={p.id} style={s.projCard}>
          <div style={{ flex: 1 }}>
            <p style={{ margin: "0 0 4px", color: "#e8f0fe", fontSize: 15 }}>{p.nome}</p>
            <p style={{ margin: 0, color: "#4a6f8a", fontSize: 11 }}>{p.tipo}</p>
          </div>
          <button style={{ ...s.statusBadge, background: STATUS_COLORS[p.status] + "33", color: STATUS_COLORS[p.status], borderColor: STATUS_COLORS[p.status] }} onClick={() => nextStatus(p.id)}>{p.status} →</button>
      ...
      {insumos.map(i => (
        <div key={i.id} style={{ ...s.txRow, alignItems: "center" }}>
          <div style={{ flex: 1 }}><p style={{ margin: 0, color: "#e8f0fe", fontSize: 14 }}>{i.nome}</p></div>
          <p style={{ margin: "0 12px 0 0", color: "#C5B358" }}>Qtd: {i.qtd}</p>
      ...
      {modalProj && (
        <Modal title={editProj ? "Editar Projeto" : "Novo Projeto"} onClose={() => setModalProj(false)}>
          ...
          <select style={s.select} value={formProj.tipo} onChange={e => setFormProj({ ...formProj, tipo: e.target.value })}>
            <option>Relógio de Resina</option><option>Camiseta</option><option>Outro</option>
          </select>
...
function TabIdiomas() {
  ...
  const save = () => {
    if (!form.pt) return;
    const upd = editCard ? cards.map(c => c.id === editCard.id ? { ...c, ...form } : c) : [{ id: uid(), ...form, data: hoje, arquivado: false }, ...cards];
    setCards(upd); DB.set("idiomas", upd); setModal(false); setEditCard(null);
  };
  const arquivar = (id) => { const upd = cards.map(c => c.id === id ? { ...c, arquivado: true } : c); setCards(upd); DB.set("idiomas", upd); };
    ...
    <div style={s.tabWrap}>
      ...
      {revisao.length > 0 && (
        <div style={s.revisaoBox}>
          <p style={{ color: "#C5B358", fontFamily: "Georgia, serif", margin: "0 0 8px", fontSize: 13 }}>✦ Revisão de ontem ({revisao.length} frases)</p>
          {revisao.map(c => <div key={c.id} style={{ color: "#8aadcf", fontSize: 12, marginBottom: 4 }}><strong style={{ color: "#e8f0fe" }}>{c.pt}</strong> → {c.en || c.th}</div>)}
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {ativos.map(c => (
          <div key={c.id} style={{ ...s.flashCard, background: flip[c.id] ? "#1a3a5c" : "#0f2744" }} onClick={() => toggleFlip(c.id)}>
            <p style={{ color: "#C5B358", fontSize: 10, letterSpacing: "0.1em", margin: "0 0 6px" }}>{c.idioma} · toque para virar</p>
            {flip[c.id] ? <p style={{ color: "#ffd700", fontSize: 14, margin: "0 0 6px" }}>{c.en || c.th}</p> : <p style={{ color: "#e8f0fe", fontSize: 14, margin: "0 0 6px" }}>{c.pt}</p>}
      ...
      {modal && (
        <Modal title={editCard ? "Editar Flashcard" : "Novo Flashcard"} onClose={() => setModal(false)}>
          ...
          <select style={s.select} value={form.idioma} onChange={e => setForm({ ...form, idioma: e.target.value })}><option>Inglês</option><option>Tailandês</option></select>
          ...
          <input style={s.input} value={form.idioma === "Inglês" ? form.en : form.th} onChange={e => setForm(form.idioma === "Inglês" ? { ...form, en: e.target.value } : { ...form, th: e.target.value })} placeholder={`Tradução em ${form.idioma}...`} />
...
function TabMentor() {
  ...
  const send = async () => {
    ...
    setMsgs(newMsgs); DB.set("chat", newMsgs); setInput(""); setLoading(true);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "Você é o Mentor IA do Golden Self-Alchemy. Trate sempre a usuária como 'amiga'. Seja encorajador, focado em alta performance e psicologia financeira (escassez → abundância). Responda em português do Brasil.",
          messages: newMsgs.map(m => ({ role: m.role, content: m.content }))
        })
      });
      const data = await res.json();
      const reply = data.content?.[0]?.text || "Não consegui responder agora, amiga.";
      const updated = [...newMsgs, { role: "assistant", content: reply }];
      setMsgs(updated); DB.set("chat", updated);
    } catch {
      const updated = [...newMsgs, { role: "assistant", content: "Erro de conexão, amiga. Tente novamente." }];
      setMsgs(updated); DB.set("chat", updated);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ ...s.tabWrap, paddingBottom: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <button style={{ ...s.btnAdd, fontSize: 11 }} onClick={() => { const r = [msgs[0]]; setMsgs(r); DB.set("chat", r); }}>Limpar</button>
      </div>
      <div style={s.chatBox}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 10 }}>
            <div style={{ ...s.bubble, ...(m.role === "user" ? s.bubbleUser : s.bubbleAssist) }}>{m.content}</div>
          </div>
        ))}
        {loading && <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 10 }}><div style={s.bubbleAssist}><span style={{ color: "#C5B358" }}>✦ ✦ ✦</span></div></div>}
        <div ref={bottomRef} />
      </div>
      <div style={s.chatInput}>
        <input style={s.chatField} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Escreva aqui, amiga..." />
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      setTimeout(() => { insightIdx.current = (insightIdx.current + 1) % INSIGHTS.length; setInsight(INSIGHTS[insightIdx.current]); setInsightVisible(true); }, 500);
    }, 7000);
      ...
      {sidebarOpen && (
        <div style={s.sideOverlay} onClick={() => setSidebarOpen(false)}>
          <div style={s.sidebar} onClick={e => e.stopPropagation()}>
            ...
            {TABS.map(t => (
              <button key={t.id} style={{ ...s.sideItem, ...(tab === t.id ? s.sideItemActive : {}) }} onClick={() => { setTab(t.id); setSidebarOpen(false); }}>
